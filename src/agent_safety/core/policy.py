...
from .gates import ApprovalGate, ConstitutionGate, PreviewGate, ReasoningGate
from .guards import Guard, Stage, run_guards
from .limits import ConcurrencyLimit, Deadline, LoopGuard, RateLimit
from .permissions import PermissionSet
from .quota import CostBudget, QuotaLike, RiskBudget
...
class Policy:
    ...
    def audit(self, event: AuditEvent) -> None:
        ...
        try:
            from ..distributed.run import current_run
            run = current_run()
            if run is not None:
    ...
    def check_approval(self, request: Action) -> None:
    ...
    async def check_approval_async(self, request: Action) -> None:
    ...
    @staticmethod
    def _request_for(request: Action, gate: ApprovalGate) -> Action:
        if gate.reason and not request.reason:
            return replace(request, reason=gate.reason)
        return request

    def _decide_approval(
        self, request: Action, gate: ApprovalGate, approved: bool
    ) -> None:
    ...
    def check_constitution(self, request: Action) -> None:
    ...
    async def check_constitution_async(self, request: Action) -> None:
    ...
    def _decide_constitution(self, request: Action, rule: str, ok: bool) -> None:
        if ok:
    ...
    def check_preview(self, request: Action, preview: str) -> None:
    ...
    async def check_preview_async(self, request: Action, preview: str) -> None:
    ...
    def _decide_preview(self, request: Action, preview: str, ok: bool) -> None:
        self.audit(AuditEvent(
    ...
    def check_reasoning(self, request: Action, rationale: Optional[str]) -> None:
