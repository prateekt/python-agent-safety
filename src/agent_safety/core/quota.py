...
@runtime_checkable
class QuotaLike(Protocol):
    """Anything a :class:`~agent_safety.policy.Policy` can charge calls/tokens to.

    Implemented by the local :class:`Quota` and by
    :class:`~agent_safety.backends.BackendQuota` (which forwards charges to a
    shared budget store) — the policy engine treats them identically.
    """

    def charge_call(self, n: int = 1) -> None:
        """Account for *n* calls, raising a safety exception when over budget."""
        ...

    def charge_tokens(self, n: int) -> None:
        """Account for *n* tokens, raising a safety exception when over budget."""
        ...
